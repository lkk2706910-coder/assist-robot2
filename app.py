...
def load_data():
   try:
       ...
       # 這裡會捕捉到圖片中的 Spreadsheet must be specified 錯誤
       st.error(f"讀取雲端失敗: {e}")
...
def solve_schedule(year, month, g1_cfg, g2_cfg, p_df, m_df, l_df):
   ...
   for d in days:
       ...
       if len(g1_cfg['r']) == 2 and g1_cfg['r'][0] <= curr <= g1_cfg['r'][1]:
           ...
           dn = g2_cfg['p'][(curr - g2_cfg['r'][0]).days // 2 % len(g2_cfg['r'])]
       if dn:
   ...
   for e in all_staff:
       ...
       if staff_night_count[e] > 20:
           ...
           # 第二階段：盡量達成 2 天 (中高權重)
           h2w = model.NewBoolVar(f'h2w_{e}'); model.Add(w_cnt >= 2).OnlyEnforceIf(h2w); soft_penalties.append(h2w.Not() * 100000)
           h2h = model.NewBoolVar(f'h2h_{e}'); model.Add(h_cnt >= 2).OnlyEnforceIf(h2h); soft_penalties.append(h2h.Not() * 100000)
           # 第三階段：第 3 天給予懲罰 (能省則省)
           i3w = model.NewBoolVar(f'i3w_{e}'); model.Add(w_cnt == 3).OnlyEnforceIf(i3w); soft_penalties.append(i3w * 5000)
           i3h = model.NewBoolVar(f'i3h_{e}'); model.Add(h_cnt == 3).OnlyEnforceIf(i3h); soft_penalties.append(i3h * 5000)
       model.Add(w_cnt <= 3); model.Add(h_cnt <= 3)
   # C. 日值班基本約束
