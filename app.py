...
try:
    ...
    st.error("⚠️ 無法建立 Google Sheets 連線，請檢查 Secrets 設定。")
...
if st.button("🚀 執行優化排班"):
    ...
    if status in [cp_model.OPTIMAL, cp_model.FEASIBLE]:
        ...
        stats = {e: {"平日": sum(solver.Value(x[(e, d, 0)]) for d in w_set), "假日": sum(solver.Value(x[(e, d, 0)]) for d in h_set), "大夜": sum(solver.Value(x[(e, d, 1)]) for d in range(1, last_day+1))} for e in all_staff}
