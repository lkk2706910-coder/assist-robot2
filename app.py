...
if "h_set" not in st.session_state: st.session_state.h_set = set()
...
with st.sidebar:
    ...
    with st.expander("🚩 2. 自訂假日"):
        edited_holiday = st.data_editor(st.session_state.holiday_data, num_rows="dynamic", key="h_ed", column_config={"日期": st.column_config.DateColumn()})
    ...
    with st.expander("📅 4. 固定會議"):
        edited_m = st.data_editor(st.session_state.m_data, num_rows="dynamic", key="m_ed",
            column_config={"人員": st.column_config.SelectboxColumn(options=current_staff_list), "開會時間": st.column_config.SelectboxColumn(options=week_list)})
    with st.expander("🏖️ 5. 人員休假"):
        edited_leave = st.data_editor(st.session_state.leave_data, num_rows="dynamic", key="l_ed",
            column_config={"人員": st.column_config.SelectboxColumn(options=current_staff_list), "開始日期": st.column_config.DateColumn(), "結束日期": st.column_config.DateColumn()})
    ...
    if st.button("💾 儲存設定並刷新"):
        try:
            with st.spinner("同步中..."):
                ...
                if not h_save.empty: h_save["日期"] = h_save["日期"].astype(str)
...
def solve_schedule(year, month, staff_list, g1_cfg, g2_cfg, p_df, m_df, l_df, h_df):
    ...
    for d in days:
        ...
        if len(g1_cfg['p']) > 0 and len(g1_cfg['r']) == 2 and g1_cfg['r'][0] <= curr <= g1_cfg['r'][1]:
            current_g_p = g1_cfg['p']
            current_g_r = g1_cfg['r']
            # 指定當天值大夜的人
            ...
            current_g_p = g2_cfg['p']
            current_g_r = g2_cfg['r']
            dn = g2_cfg['p'][((curr - g2_cfg['r'][0]).days // 2) % len(g2_cfg['p'])]
            model.Add(x[(dn, d, 1)] == 1)

    # 實作：組員在整個大夜期間及其前後 3 天禁止白班
    for e in staff_list:
        for d in days:
            curr = date(year, month, d)
            
            # 檢查是否在第一組大夜的影響範圍內
            if len(g1_cfg['p']) > 0 and e in g1_cfg['p']:
                start_limit = g1_cfg['r'][0] - timedelta(days=3)
                end_limit = g1_cfg['r'][1] + timedelta(days=3)
                if start_limit <= curr <= end_limit:
                    model.Add(x[(e, d, 0)] == 0)
            
            # 檢查是否在第二組大夜的影響範圍內
            if len(g2_cfg['p']) > 0 and e in g2_cfg['p']:
                start_limit = g2_cfg['r'][0] - timedelta(days=3)
                end_limit = g2_cfg['r'][1] + timedelta(days=3)
                if start_limit <= curr <= end_limit:
                    model.Add(x[(e, d, 0)] == 0)

    # 基礎人數限制
    for d in days:
        model.Add(sum(x[(e, d, 1)] for e in staff_list) == 1) # 每天 1 個大夜
        need = 2 if d in h_set else 1
        model.Add(sum(x[(e, d, 0)] for e in staff_list) == need) # 日值班人數

    # 其他限制：休假、會議、偏好 (省略重複代碼，邏輯同前)
    for e in staff_list:
        ...
        for _, row in emp_p.iterrows():
            ...
            for d in target_days:
                if row["限定班別"] == "不值班": model.Add(x[(e, d, 0)] == 0)
                    ...
                    if get_ab_shift(date(year, month, d)) != row["限定班別"]: model.Add(x[(e, d, 0)] == 0)
    ...
    for e in staff_list:
        ...
        for goal, weight in [(1, 5000), (2, 500)]:
            b_w = model.NewBoolVar(f"w_{e}_{goal}"); model.Add(w_cnt >= goal).OnlyEnforceIf(b_w); soft_penalties.append(b_w.Not() * weight)
            b_h = model.NewBoolVar(f"h_{e}_{goal}"); model.Add(h_cnt >= goal).OnlyEnforceIf(b_h); soft_penalties.append(b_h.Not() * weight)
...
# --- 5. 主畫面 (其餘代碼與 session_state 邏輯相同) ---
    ...
    if st.button("🚀 執行優化排班"):
        ...
        if status in (cp_model.OPTIMAL, cp_model.FEASIBLE):
            ...
            st.session_state.df_stats = pd.DataFrame([{
                "人員": e, "平日值班": sum(solver.Value(x[(e, d, 0)]) for d in w_set),
            ...
            st.success("✅ 排班成功！已自動避開大夜人員衝突。")
        else:
            st.error("❌ 找不到可行方案。請確保大夜班以外的人力足以應付白班。")

    if st.session_state.df_res is not None:
        def highlight_holiday(row):
            day_num = int(row["日期"].split("/")[-1])
            return ["background-color: #FFF9C4"] * len(row) if day_num in st.session_state.h_set else [""] * len(row)
        st.subheader("🗓️ 排班結果明細")
        st.write(st.session_state.df_res.style.apply(highlight_holiday, axis=1).to_html(), unsafe_allow_html=True)
        st.subheader("📊 本月統計")
        st.dataframe(st.session_state.df_stats, use_container_width=True)
        # 上傳按鈕與邏輯省略，同前...
