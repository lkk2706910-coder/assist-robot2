...
def get_node_angle(node_name, cass_name, side, start_angle):
    """
    根據使用者更新的角度規則：
    A/B S1: LL1(195), CHA1(15), CHB1(285), CHC1(105)
    A/B S2: CS(255), LL2(345), CHA2(165), CHB2(75), CHC2(255)
    C/D S1: CS(105), LL1(15), CHA1(195), CHB1(105), CHC1(285)
    C/D S2: LL2(165), CHA2(345), CHB2(255), CHC2(75)
    """
    ...
    if cass_name in ("A", "B"):
        if side == "S1":
            rules = {"LL1": 195, "A1": 15, "B1": 285, "C1": 105, "CS": 0}
        else: # S2
            rules = {"CS": 255, "LL2": 345, "A2": 165, "B2": 75, "C2": 255}
    else: # C/D
        if side == "S1":
            rules = {"CS": 105, "LL1": 15, "A1": 195, "B1": 105, "C1": 285}
        else: # S2
            rules = {"LL2": 165, "A2": 345, "B2": 255, "C2": 75, "CS": 0}
...
def refresh_ui(info_text, canvas, c_var, st_var, prefix=""):
    ...
    msg = f"{prefix}\n[進站角度]: {base_angle}° | [路徑角度]:\n" + " -> ".join([f"{n}({get_node_angle(n, sel_c, current_side, base_angle)}°)" for n in path])
...
def main():
    root = tk.Tk()
    root.title("Wafer Visualizer v5.3 - Updated Angles")
